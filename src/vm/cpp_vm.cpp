...
namespace context {
  ...
  /* frame */
  frame::frame(context *prnt, frame *p, int s) :
    parent(prnt),
    prev(p),
    next(0),
    size(s),
   ...
   */
  frame * context::push_new_frame(int size) {
  ...
  void context::push_on(stack_entry s) {
    ...
    if(bottom->sp < 1) {
      throw frame_overflow("");
    }
    *(bottom->u + --bottom->sp) = s;
  }

  stack_entry context::pop_off(bool zero) {
    if (bottom->sp == bottom->size) {
      throw frame_underflow("");
    }
